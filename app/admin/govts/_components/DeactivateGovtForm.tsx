...
//   3. On submit, attachmentIds[] are passed to deactivateGovtAction.
//
...
export function DeactivateGovtActions({
  ...
  return (
    <button
      ...
      className="text-xs px-3 py-1.5 rounded-md border border-red-300 text-red-800 hover:opacity-90 transition-opacity"
    >
      Desactivar govt
    </button>
  );
...
function DeactivateGovtForm({
  ...
  return (
    <div className="rounded border border-red-200 p-3 space-y-3 bg-gob-danger/10">
      <p className="text-xs uppercase tracking-wider text-gob-danger">
        Desactivar govt — {target.displayName}
      </p>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={submit}
          disabled={!canSubmit}
          className="text-xs px-3 py-1.5 rounded-md bg-red-700 text-white hover:opacity-90 disabled:opacity-50"
        >
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <label
          htmlFor="deactivate-govt-motivo"
          className="block text-[10px] uppercase tracking-wider text-gob-text-muted"
        >
