...
async function AdminPanoramaBoard({
  ...
  if (seedPreset) {
    ...
    // the aggregate cube, surface the cube's build time so an operator can tell
    // data lagging a day from no data at all. Read-only — resolveCubeFreshness
    // reads only the cube meta row, never re-assembles a layer. Null (stamp
    // omitted) for a live-served or points-only preset.
