...
vi.mock("react", async (importOriginal) => {
  ...
  return {
    ...actual,
    // biome-ignore lint/suspicious/noExplicitAny: test stub, matches finder-in-possession-form.test.tsx pattern.
...
describe.each(FORMS)("<$name> — warning copy, not impact-preview gate (decision #651)", ({ Component }) => {
  beforeEach(() => {
  ...
  it("renders the explicit warning copy near the submit button", () => {
    ...
    expect(html).toContain("Este cambio aplica inmediatamente a toda la jurisdicción seleccionada.");
  });
  ...
  it("renders the submit button enabled (not gated on an acknowledgement)", () => {
    ...
    expect(buttonMatch?.[0]).not.toMatch(/\sdisabled(=|\s|>)/);
  });
});
