    ...
    <SafeAreaView style={styles.safe} edges={["bottom"]}>
      ...
      >
        ...
        <View style={styles.footer}>
          <PrimaryButton label="Ajustes" tone="quiet" onPress={() => router.push(ROUTES.ajustes)} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
function StatusChip({ status }: { status: MyPetsV1Item["status"] }) {
  ...
  return (
    <View style={[styles.chip, tone]}>
      <Text style={label}>{petStatusLabel(status)}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  photo: { width: 52, height: 52, borderRadius: RADIUS.md, backgroundColor: COLORS.canvas },
  photoFallback: {
    width: 52,
    height: 52,
  ...
  photoFallbackText: { fontSize: 10, color: COLORS.inkMuted },
  petText: { flex: 1, gap: 2 },
  petName: { fontSize: 17, fontWeight: "700", color: COLORS.ink },
  petSpecies: { fontSize: 13, color: COLORS.inkMuted },
  chip: { borderRadius: RADIUS.sm, paddingHorizontal: SPACE.sm, paddingVertical: SPACE.xs },
  chipQuiet: { backgroundColor: COLORS.canvas },
  chipAlert: { backgroundColor: COLORS.dangerSurface },
  chipQuietLabel: { fontSize: 12, color: COLORS.inkSoft, fontWeight: "600" },
  chipAlertLabel: { fontSize: 12, color: COLORS.danger, fontWeight: "700" },
});
