...
/**
 * The person this draft belongs to, or `null` when nobody is signed in.
 *
 * READ PER RENDER AND NOT SUBSCRIBED TO, deliberately. The route that mounts
 * this screen is behind `useGate`, so the session is already resolved and
 * `signed-in` before the form draws its first field; there is no transition to
 * wait for. Reading it each render is a property access, and if the session
 * DOES end mid-form the key becomes `null` and every writer below no-ops, which
 * is the correct behaviour rather than a stale key that keeps being written.
 */
...
export function useEventDraft({
  ...
  const persistNow = useCallback(() => {
    ...
    queue.current = queue.current.then(() => writeEventDraft(key, values));
  }, [key]);

  const forget = useCallback(() => {
    if (key === null) return;
    persisted.current = null;
    queue.current = queue.current.then(() => forgetEventDraft(key));
  }, [key]);
