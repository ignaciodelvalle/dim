 ...
 *
 * NOT WIRED TO SIGN-OUT, and the reason is worth recording because the obvious
 * move is to put it next to `forgetAllCachedCredentials()` in `clearSession()`.
 * `clearSession` is the funnel EVERY end of a session passes through, including
 * the one nobody chose: `accessToken()`'s refused arm calls it when a refresh
 * is rejected. Sweeping there would mean an auth blip mid-form destroys the
 * text somebody is writing — turning the exact loss this whole module exists to
 * prevent into a feature of the fix. The confidentiality that a sweep would buy
 * is already bought by the owner id in the key: another person's session cannot
 * name, let alone read, a draft that is not theirs.
 *
 * So it is exported and unused, for a maintainer or a PO who decides the
 * shared-phone case is worth that cost on a DELIBERATE sign-out only.
 */
