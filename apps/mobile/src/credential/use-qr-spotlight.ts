...
export function useQrSpotlight(enabled = true): void {
  // Deactivates itself on unmount; separate from the brightness effect because
  // keep-awake has no state to capture or restore.
  //
  // KNOWN ASYMMETRY, left deliberately: unlike the brightness below, keep-awake
  // is still tied to MOUNT, so it stays on under a pushed screen. That is a
  // battery cost, not a usability defect — nobody's phone stops behaving the
  // way they set it up — and gating it would mean driving
  // `activateKeepAwakeAsync`/`deactivateKeepAwake` by hand, since a hook cannot
  // be called conditionally. Worth doing, not worth folding into a fix for a
  // different bug.
  useKeepAwake();
