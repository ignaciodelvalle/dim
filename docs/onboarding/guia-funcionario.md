...
> Guía de arranque para el área de zoonosis, bromatología o bienestar animal de un municipio. Si venís de un organismo **nacional** o provincial y lo que buscás es mirar sin intervenir, saltá a *"Si sos de un organismo nacional"*.
...
### Si sos de un organismo nacional: hay una cuenta de sólo lectura

Existe un tipo de cuenta institucional pensado para un organismo **nacional** que necesita mirar el país entero sin intervenir en la gestión de ningún municipio. Es muy nueva, y conviene decir primero lo menos cómodo: **todavía no hay una pantalla para darla de alta**. Hoy la crea el equipo técnico de miMAR a mano, a pedido — no se puede solicitar desde el producto, ni la puede crear un administrador desde su portal. Si tu organismo la necesita, es una conversación con el equipo.

Con eso dicho, así funciona:

- **Ve todo el país**, no una lista de localidades. No se le asigna jurisdicción porque su alcance es nacional.
- **Es de sólo lectura, y eso está garantizado por diseño**: no aprueba una matrícula, no asigna una denuncia, no cierra un caso, no configura una regla. Cada acción que modifica algo está cerrada para esta cuenta.
- **Entra al mismo portal de gobierno** y lee el briefing, el panorama, los tableros del programa, la vigilancia epidemiológica y los brotes, las denuncias, las adopciones, las mascotas perdidas, la mortalidad, "Acciones que vencen", los casos, la cola ENO y el listado de la cola de aprobaciones (el listado; el detalle de cada solicitud no, ver abajo).
- **No entra al portal de administración del sistema.**

Lo que todavía no alcanza, dicho de frente: **su terreno son los tableros y las colas en agregado, no el expediente individual**. A esta cuenta la rebotan la configuración de **Reglas**, la **ficha individual de una mascota**, el **detalle de una solicitud** de la cola de aprobaciones, el detalle de un **servicio**, las **observaciones antirrábicas**, el alta de una **investigación** nueva, y las **descargas en planilla** (CSV) de los tableros. Tampoco tiene el **buscador general** del portal — ése busca personas y mascotas por nombre o documento, y es justamente el dato individual que esta cuenta no está pensada para ver. Es una decisión de privacidad, no una funcionalidad faltante.
...
4. **Abrí "Denuncias"** (misma sección). Es la mesa de entradas de los reportes de maltrato y abandono de tu territorio (Ley 14.346), con dos etapas: moderación de denuncias anónimas y triage de casos — asignar, priorizar, seguir y cerrar. Las denuncias llegan con tipo, gravedad, ubicación y evidencia adjunta si la persona la cargó.
   > **Una mordedura no es una denuncia y viaja por otro lado.** Se registra como un hecho sanitario en la libreta del animal —lo puede hacer el dueño, desde la web o desde el celular, o un veterinario— y abre un **expediente** con número `CAS-XXXX-XXXX` que la persona puede citarte. El formulario pregunta **dónde ocurrió la mordedura**, no dónde está registrado el animal: un perro con domicilio en otra jurisdicción que muerde en tu localidad es un caso tuyo.
...
7. **Mirá "Acciones que vencen"** (sección *Bandeja operativa*). Es una sola lista con todo lo que tiene un plazo corriendo en tu jurisdicción, ordenado por vencimiento y sin importar de qué módulo venga: observaciones antirrábicas abiertas, denuncias de maltrato abiertas y casos regulatorios. Si tenés cinco minutos por día para miMAR, gastalos acá.
8. **Abrí la "Cola ENO"** (sección *Situación*; es la Bandeja de salida con la vista de avisos legales). Son las notificaciones con **plazo legal** hacia la autoridad sanitaria y hacia tu sistema receptor, **ordenadas por vencimiento: primero las ya vencidas**. Cada fila trae la enfermedad, el plazo legal y el estado del plazo. Podés acotar por **vista, estado, destino, SLA** (sólo incumplimientos / sólo dentro del plazo) **y provincia**.
   > **Leé bien el estado de una fila, porque acá se puede malinterpretar algo importante.** El estado describe lo que hizo *nuestro* sistema, no lo que recibió la autoridad externa. Vas a ver leyendas del tipo *"Registrada y auditada — transmisión a la autoridad pendiente de endpoint receptor"*, y eso es literal: el aviso quedó registrado y auditado con su plazo, y **no hay todavía un receptor conectado al que mandarlo**. Esta pantalla es tu tablero de control de vencimientos; no es un acuse de recibo de nadie.
9. **Revisá "Observaciones"** (sección *Situación*) si en tu jurisdicción corre el período de observación antirrábica de 10 días. Ahí se abren, se siguen y se cierran, con los vencimientos a la vista.
10. **Si tu jurisdicción tiene reglas propias** (por ejemplo, sobre razas potencialmente peligrosas o canales de credencial física), revisá **"Reglas"** (sección *Profundidad*): ahí se configuran las reglas de negocio que aplican en tu territorio. Lo que definas para tu localidad pisa el valor por defecto del país.

Lo que queda del menú, para que no te tome por sorpresa: **Vigilancia** (brotes e investigaciones epidemiológicas) y **Pérdidas** están en *Situación*; **Operativos** (campañas y alcance comunitario) y **Decomisos** arman la sección *Intervención*; **Casos**, **Bandeja de salida** y **Alertas y suscripciones** completan la *Bandeja operativa*; y en *Profundidad* están **Historial**, **Reglas** y el **Directorio** (organizaciones, usuarios, servicios y credenciales, todo junto).

Resultado concreto al final del recorrido: sabés qué está esperando una decisión de tu área, sabés qué vence primero, viste el estado sanitario de tu territorio en el mapa, y sabés dónde llegan las denuncias de tu jurisdicción.
...
- **No envía denuncias ni notificaciones sanitarias a sistemas estatales externos.** Las denuncias se gestionan dentro de miMAR (tu área las ve en su bandeja); la derivación automática a canales gubernamentales externos está en desarrollo. Lo mismo vale para los eventos de declaración obligatoria: la **Cola ENO** que ves es una pantalla de control de tus propios plazos —te dice qué está por vencer y qué ya venció— pero **el envío automático al organismo receptor todavía no está conectado**, y en esa pantalla no hay ningún botón para despachar un aviso. Que una fila figure ahí no significa que la autoridad externa la haya recibido.
...
- **Tu superficie de trabajo es web, y es a propósito.** Los tableros, las bandejas y el panorama de tu jurisdicción están pensados para una pantalla grande y no tienen aplicación para instalar. Sí existe una app Android para dueños de mascotas, hoy en **prueba cerrada** (por invitación, con un grupo chico de personas; no hay versión para iPhone). No la vas a necesitar para tu tarea, pero conviene que sepas que buena parte del dato que ves puede haber entrado desde un teléfono: hoy el dueño puede registrar su mascota, anotar y corregir casi toda su libreta, reportar una mordedura y activar el modo perdida desde ahí.
