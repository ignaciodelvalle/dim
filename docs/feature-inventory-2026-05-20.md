...
| 🟡 partial | PPP export provincial Prov BA (CABA shipped en Chunk F) · Admin page final (4 roles) |
| 🔴 deprecado | `/pro` portal (2026-05-20) — los vets profesionales ahora operan vía clinic org de 1 miembro |
| ⚪ planeado | Mi Argentina OAuth · DNI verification real · Bulk ops (Chunk G — bloqueada por B2 owner-side) · 3 case_kinds deferidos restantes (`custody_episode`, `foster_proposal`, `outbreak_investigation`) — `microchip_remediation` se activa con el plan de §3.3.21 |
| ❎ diferido | Wizard adopción 28-preguntas (`docs/superpowers/plans/2026-05-20-adoption-handshake-unified.md` queda en backlog hasta nueva demanda) · **Chunk N** (Iconic-dataset cleanup — audit dice 0 phases tocan UI, ship como `chore:` cuando se quiera demo viva) · **PPP Prov BA v2** (CABA shipped; municipal heterogeneidad relega Prov BA a v2 — TODO(F2-prov-ba-v2) en `lib/ppp-exports.ts`) |
