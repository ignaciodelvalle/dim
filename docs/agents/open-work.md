...
2. **STILL OPEN, and now due** — photo upload ships in the live build (`a32dc71ce`), and production review starts ~2026-09-26. On 2026-09-13 the PO believed it was already declared; the Console's Data safety wizard hung on every step change in automation, so it is UNVERIFIED either way. Check Data types → Photos and videos → Photos. Revise the **Data Safety** form before any build with uploads reaches Play. It declared on 27/08 that the app does not collect photos; that stops being true the moment uploads ship, and a form that no longer matches the binary is a policy violation by itself.
