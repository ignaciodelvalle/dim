...
| **Leaving the public credential leaves the app holding the phone's brightness** | tester-reported 2026-09-12 — fix agreed by PO, scheduled in a few days; OTA-shippable (JS only) | `apps/mobile/src/credential/use-qr-spotlight.ts:47` "restores" by calling `setBrightnessAsync(previous)`. On Android that writes a FIXED window value (`BrightnessModule.kt:36`, `lp.screenBrightness = value`) instead of clearing the override, so after leaving the screen the activity keeps pinning brightness until the app is killed: Settings says an app controls it, the slider does not move, auto-brightness is off. `getBrightnessAsync` reads the system level when no override is set (`BrightnessModule.kt:42-46`), which is why `previous` looks correct and the bug hides. **Agreed fix:** on Android restore with `restoreSystemBrightnessAsync()` (`BRIGHTNESS_OVERRIDE_NONE`, `BrightnessModule.kt:79`); raise on focus and release on blur (`useFocusEffect`) and on backgrounding (`AppState`), not only on unmount — a native-stack screen stays mounted under a pushed screen. Keep `setBrightnessAsync(previous)` for iOS only. |
...
| **Password recovery started from the WEB cannot be completed** | serious — found 2026-09-13; affects every web user, including the vets being onboarded; fix is a dashboard template edit (PO) | The Supabase "Reset password" template (DIM-staging → Authentication → Emails) contains only `{{ .Token }}` and says "Ingresá este código en la app". The web flow is link-based: `request-password-reset.ts:160` calls `resetPasswordForEmail(email, { redirectTo })`, the button reads "Enviar enlace de recuperación" (`ResetRequestForm.tsx:44`), and `/recuperar/actualizar` expects a session from `/auth/callback` (`exchangeCodeForSession`) — there is no code field anywhere under `app/(auth)/recuperar`. A web user therefore receives a code with nowhere to type it. **PO decision 2026-09-13: ONE method, the 6-digit code, on both surfaces** — scheduled in a few days. A dual template (code + `{{ .ConfirmationURL }}` button) was considered and REJECTED: the PKCE link only works in the browser that requested it (the web already carries an error state for "recovery link opened on a different device", `recuperar/page.tsx:26`), mail scanners can consume a one-time link, and an app-side link would need deep links that are still blocked. **Fix shape:** the web's `/recuperar` gets a code step mirroring the app — after the request, a 6-digit field that verifies with `verifyOtp({ email, token, type: "recovery" })` server-side, then lands on `/recuperar/actualizar` with the recovery session; the button copy stops saying "enlace". Keep the template code-only. Remove the link-only callback path for recovery once nothing emits links. Verify from both surfaces and from a second device. |
