...
| **Leaving the public credential leaves the app holding the phone's brightness** | tester-reported 2026-09-12 — **MEASURED on a real device 2026-09-16**, and the measurement narrows the row; fix agreed by PO; OTA-shippable (JS only) | `apps/mobile/src/credential/use-qr-spotlight.ts` "restores" by calling `setBrightnessAsync(previous)`. On Android that writes a FIXED window value (`BrightnessModule.kt:36`, `lp.screenBrightness = value`) instead of clearing the override. `getBrightnessAsync` reads the system level when no override is set (`BrightnessModule.kt:42-46`), which is why `previous` looks correct and the bug hides. **Measured on a Samsung SM-J710MN, Android 8.1, via `dumpsys power`** — the user's own `mScreenBrightnessSetting` stayed at 148 throughout, and `mScreenBrightnessOverrideFromWindowManager` went: **255** with the credential open (correct: window-level, system untouched) → **147** after turning the in-screen control off (WRONG: should be -1, released; it pinned the captured value instead) → **147** after leaving the screen → **-1** only once the app went to background. **So the earlier wording was too broad**: it said the activity keeps pinning brightness "until the app is killed". It does not — backgrounding releases it, because the override is per-window. The real defect is bounded to the session INSIDE the app: from opening the credential until leaving the app, auto-brightness is overridden. That is invisible on a phone set to manual (this tester's is) and is the whole problem on a phone set to automatic, which is the common case: open the credential in the street, go back, and the screen stops adapting for the rest of the visit. **Agreed fix:** on Android restore with `restoreSystemBrightnessAsync()` (`BRIGHTNESS_OVERRIDE_NONE`, `BrightnessModule.kt:79`); raise on focus and release on blur (`useFocusEffect`) and on backgrounding (`AppState`), not only on unmount — a native-stack screen stays mounted under a pushed screen. Keep `setBrightnessAsync(previous)` for iOS only. **Note the new neighbour:** the owner can now decline the spotlight entirely (`qr-spotlight-preference.ts`, 2026-09-16), and turning it off mid-screen goes through this same broken restore — so the opt-out currently swaps a bright pin for a dim one rather than handing control back. |
