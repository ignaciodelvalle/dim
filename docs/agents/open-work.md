...
**The table holds THREE rows — 1, 4 and 6 — and the count, not the highest
number, is the thing to quote.** Two lanes removed a row each in the same window
and each wrote "the table holds four", which was true of the tree each one held
and false of the tree you are reading. That is the fourth time a count in this
preamble has rotted inside a single window, and the first time it rotted while
being corrected: the paragraph three below is a lane fixing this exact sentence,
and its fix was stale before it was committed.
...
| **Sign-in authenticates but the session never persists on Android** | **blocks a live tester** | Measured 2026-08-31 on the FIRST run of this app on a real Android device (emulator `sdk_gphone64_arm64`, API 36, dev build via `expo run:android`, `EXPO_PUBLIC_SUPABASE_*` pointed at `http://10.0.2.2:54321`). With correct credentials the screen reports **"Iniciaste sesión, pero no pudimos guardarla en este dispositivo"** — so GoTrue accepted the password and the failure is downstream, in `createSecureStoreAuthStorage()`. **This is NOT the build-5 problem** (that one shipped without the env vars and could not authenticate at all); here authentication succeeds. Reproduced twice. **One hypothesis was tested and REFUTED**: the emulator had no lock screen, so the Android Keystore was suspected — `locksettings set-pin 1234` and a fresh launch reproduced the same error, so the missing device credential is not the cause. Not root-caused: `adb logcat` carried no `SecureStore` or `expo.modules.securestore` line, and the chunking adapter's own header documents the 2048-byte limit as handled. Next step for whoever picks it up: instrument the adapter's `setItem` and read the thrown value — the UI copy proves the write failed but nothing surfaces *why*. Note this is the first time any native screen has run outside `react-test-renderer`; the handoff lists "never ran on a device" as the largest inference in the range, and this is what the first real run found. |
