...
| 6 | **WU-P** — rehoming, foster, return, relocation, org memberships | L | **Three of five landed** (mudanza, devolucion, re-hogar — re-hogar `d8d25eb36`/`22919141e`, 2026-09-10). What remains is transito (foster) and org memberships. |

**Landed, verified 2026-09-18 — rows 8, 9, 10, 11 and 12 are DONE and come off the table above (numbering not closed up):**

- **Row 8** — `apps/mobile/src/denuncias/denuncia-view-model.ts:380-381` now sends `locationProvince`/`locationLocality`, and `app/api/v1/welfare-reports/commands.ts:293,295` consumes them. Commit `a395292f9`.
- **Row 9** — `__tests__/mobile-screen-titles.test.ts` exists and `apps/mobile/app/_layout.tsx` registers all eight turnos-class screen titles (e.g. `turnos/index` → "Mis turnos", `turnos/buscar/index` → "Buscar turno"). Commit `ff5668213`.
- **Row 10** — commit `3cc58cc83`, "confirmar y firmar preselecciona el procedimiento declarado".
- **Row 11** — `apps/mobile/src/ui/kit.tsx`'s `keyboardAvoidingBehavior()` returns `"height"` on Android; landed 2026-09-11, commit `286dc1228`, and self-cites "open-work row 11" in its own comment.
- **Row 12** — `use-scroll-to-error.ts` returns a `scrollRef` that is now passed to `<Screen scrollRef={scrollRef}>` by both callers (`DenunciaScreen.tsx:258`, `RecordEventScreen.tsx:740`).
...
**The table holds SIX rows — 1, 4, 6, 8, 9 and 10 — and the count, not
the highest number, is the thing to quote.** Row 7 (CSP dev branch) came off
2026-09-01 — see the landed block below — and the gap is left open for the
same reason 2, 3 and 5 are: these numbers are identifiers a pointer trail
depends on, not an ordering. Two lanes removed a row each in
...
### The CSP had no dev branch — landed `bb488d9c3`

`middleware.ts`'s `buildContentSecurityPolicy` now takes an explicit `dev`
parameter (default `process.env.NODE_ENV === "development"`) and appends
`'unsafe-eval'` to `script-src` ONLY on that branch — `next dev` needs it for
React Refresh, and without it the client never hydrated under `pnpm dev`
(the row above measured this A/B: flight payload present, zero `__reactFiber`
on any node). Production's policy is byte-for-byte what it was.
`__tests__/csp-dev-branch.test.ts` fences both directions: dev must carry
`'unsafe-eval'`, production must never inherit it.
...
| ~~A tester crash is unreadable~~ | **CLOSED, verified 2026-09-18** | Mobile Sentry is live: wired (commit `6939838cb`, 2026-09-01), its gradle/sentry-cli packaging issue fixed (`c7567d718`, 2026-09-03), PII redaction added before send (`12ca7d3af`, 2026-09-04), and further observability fixes landed (`f34ac2107`, 2026-09-07). A tester crash now has a DSN to report to and source maps to read it with. |
...
| **An invitee has no place to find a pending caretaker invitation** | known debt — PO 2026-09-12, no owner yet | The only doors to a pending grant are the `caretaker_invitation_received` notification (CTA to `/cuidado/{grantToken}`, `designate-caretaker.ts:94-111`) and, for an invitee without an account, the Supabase invite e-mail (`caretakers/actions.ts:180`). `GET /api/v1/me/caretaker-grants` already lists them, but no web or mobile screen renders that list for the invitee — lose the notification or the e-mail and the only recovery is asking the titular to cancel and re-invite. Transfers do not have this gap (`/transferencias` hub). **PO direction: adapt the transfers hub to also show caretaker invitations**, since both are an offer about an animal waiting on the recipient. Not folded into the native quick-capture debt: that one is about recording events, this one is about seeing offers. |
| ~~**Leaving the public credential leaves the app holding the phone's brightness**~~ | **CLOSED 2026-09-17** — tester-reported 2026-09-12, measured on a real device 2026-09-16, fixed the next day. OTA-shippable (JS only) | Shipped in `apps/mobile/src/credential/use-qr-spotlight.ts`. The old restore called `setBrightnessAsync(previous)`, which on Android writes a FIXED window value (`BrightnessModule.kt:36`, `lp.screenBrightness = value`) instead of clearing the override: it never stopped the app dictating the screen, it only MOVED the pin, from a bright one to a dim one. `getBrightnessAsync` reads the system level when no override is set (`BrightnessModule.kt:42-46`), which is why `previous` always looked correct and the bug hid. **Measured on a Samsung SM-J710MN, Android 8.1, via `dumpsys power`** — the user's own `mScreenBrightnessSetting` stayed at 148 throughout, and `mScreenBrightnessOverrideFromWindowManager` went **255** with the credential open (correct: window-level, system untouched) → **147** after turning the in-screen control off (WRONG: should be -1, released) → **147** after leaving the screen → **-1** only once the app went to background. So the defect was bounded to the session INSIDE the app: invisible on a phone set to manual (this tester's is) and the whole problem on one set to automatic. **What shipped, both halves.** WHAT it calls: a `releaseSpotlight` helper that calls `restoreSystemBrightnessAsync()` on Android and keeps `setBrightnessAsync(previous)` for iOS, where brightness IS the system brightness and there is no override to release. The Android call was verified present in `expo-brightness@57.0.1` before being used, needs no permission, and returns early on any non-Android platform (`build/Brightness.js`) — so it could NOT have been called unconditionally: that would have left every iPhone at full brightness. WHEN it fires: `useFocusEffect` (`expo-router`) and an `AppState` listener feed one `enabled && focused && !backgrounded` boolean, so blur and backgrounding release and coming back re-raises, instead of unmount-only. `inactive` is deliberately NOT a release — iOS raises it for every Control Centre pull, and flickering the screen of someone mid-conversation buys nothing. The a11y opt-out (`qr-spotlight-preference.ts`, 2026-09-16) goes through the same helper, so it now hands the phone back rather than swapping a bright pin for a dim one. **The lesson, and it is the reusable part:** the old test asserted `setBrightnessAsync(previous)` on the way out on EVERY platform, and was green for the whole time the device was leaking. It pinned what the hook DID, not what the hook had to ACHIEVE, and a restore that is indistinguishable from the bug is not a restore. The 16 cases now split by platform and were mutation-checked: reverting the platform split kills 4, removing the focus gate kills 3, removing the app-state gate kills 2. |
| **The credential keeps the screen awake under whatever is pushed on top of it** | small — noted 2026-09-17 while closing the row above; battery, not usability | `useQrSpotlight` now ties the BRIGHTNESS half to focus, but `useKeepAwake` is still tied to MOUNT, and a native-stack screen does not unmount when something is pushed over it. So opening the credential and then tapping deeper leaves the screen refusing to sleep for the rest of the visit. Deliberately left out of the brightness fix rather than missed: unlike brightness, this costs battery and nothing else — nobody's phone stops behaving the way they set it up — and it cannot be gated the same way, because a hook cannot be called conditionally. The fix is to drop `useKeepAwake` for hand-driven `activateKeepAwakeAsync`/`deactivateKeepAwake` inside the same focus window the brightness already uses, with the same best-effort try. |
| **Hardware back on the pet document leaves the screen instead of turning the card back** | tester-reported 2026-09-12 ("no se puede volver para atrás una pestaña, te vuelve a inicio") — hypothesis, not confirmed with the tester; fix agreed by PO, scheduled in a few days | The Credencial/Libreta faces are local state (`apps/mobile/src/pets/PetDocumentScreen.tsx:106`, turned by `onTurn` at `:236`), and there is no `BackHandler` anywhere in `apps/mobile/src`. On the Libreta face, back pops the whole `mascotas/[publicToken]` route and lands on "Mis mascotas", which reads as home. **Agreed fix:** while `face !== initialFace`, a focus-scoped `hardwareBackPress` listener turns the card back and swallows the event; the next back leaves the screen. Do not touch `anchor: "index"` or the gate's `<Redirect>` — sending a deep-linked screen's back to the hub is intended (NAV-1). |
| ~~**Password recovery started from the WEB cannot be completed**~~ | **CLOSED 2026-09-16** | Shipped. `app/(auth)/recuperar/ResetCodeStep.tsx` now takes the 6-digit code, and the PO's decision held: ONE method, the code, on both surfaces. Two things the original row did not anticipate. (1) The code is redeemed **in the browser**, not server-side as the fix shape proposed — server-side redemption ran every request in the country through one limiter key (`PASSWORD_RESET_VERIFY_GLOBAL_LIMIT = 4/min` on `"deployment"`), capping national recovery at 240/hour; doing it in the browser is sound because `token_verifications` is per-IP and the anon key is public anyway. (2) The password is validated BEFORE `verifyOtp`, because the code is single-use: validating after would burn the code on a typo in the new password. A failed `updateUser` destroys the recovery session (`src/modules/auth/application/password-reset/destroy-recovery-session.ts`) — `signOut` returns `{error}` without reaching `_removeSession()` (GoTrueClient.js:3192-3198), so the returned error is the signal, not an exception. |
| **A jurisdiction's rabies `frequency_months` is displayed, never computed** | serious for the government pitch — measured 2026-09-14 | `rabies_vaccination.rule_payload.frequency_months` (CABA: 12, baseline `ar-v1`) is read only to render "refuerzo cada N meses" (`lib/domain/rule-types-registry.ts:380`). The owner's compliance card derives Vigente/Por vencer/Vencida exclusively from the dose's `payload.next_due_at` or an active reminder (`lib/projections/pet-compliance.ts:469-506`); a signed dose without `next_due_at` reads **"Registrada"** forever, whatever the rule says. Measured on staging: no demo pet in CABA carries a past `next_due_at`, so the configured rule cannot currently turn any animal "Vencida". Also: no `app/gob` surface renders the per-pet card — govt sees rules and aggregate targets only. **Fix shape:** when a dose has no `next_due_at`, derive the currency from `occurred_at + resolved frequency_months` (resolver cascade already exists), keep an explicit `next_due_at` as the override, and pin it with a test where the same dose flips Vigente→Vencida by changing only the jurisdiction's rule. **Same shape, three more payload fields (re-verified 2026-09-16, whole-repo grep):** `rabies_vaccination.min_age_months` (rendered "desde los N meses", `lib/domain/rule-types-registry.ts:382-383`), `sterilization.min_age_months` ("edad mínima N meses", `:388-389`) and `sterilization.mandatory_from_months` ("aplica desde los N meses", `:390-392`). Every reference in the repo is the Zod validator (`lib/infra/business-rules-validators.ts:103-110`), the defaults table (`lib/domain/business-rules-defaults.ts:120-127`), the legal-baseline seed, the `/gob/reglas` form (`ObligationRuleForm.tsx:182-197`) and tests — **no production code reads any of the three to gate, schedule, warn or validate anything.** A funcionario configures them and the system displays them back as if they were enforced obligations. The rendering was deliberately left unchanged: the honest fix is to make one of them bite, not to soften the copy. |
| **A failed submit wipes every UNCONTROLLED field, and the class was never enumerated** | serious, public — reported by the PO 2026-09-16 while testing on a real phone | React 19 resets a `<form action={...}>` once the server action answers, **including when it answers with an error**, and the reset takes every field that is not bound to React state. Reproduced on `/p/{token}/encontre` (`FinderInPossessionForm.tsx`): a finder who had not filled the address got the validation error and lost what they had selected. Survived (controlled, `value=` + `onChange`): `finderName`, `finderPhone`, `finderEmail`, `message`, the dates. Lost (uncontrolled): the `petCondition` radio group (`<input type="radio" name="petCondition" value={value} required />`, no `checked`), `LocationFields` (used with `defaultValue`/`onChange`, so the map pin and the locality live outside React state), and the `photoNow` file input. **This is the worst surface of the class to lose**: a stranger on the street holding a lost animal, with no account and no reason to retype anything. **The method error is the point.** `MarkLostWizard.tsx` and `ApplicationForm.tsx` were fixed the same day for a different mechanism (a rejecting action escaping `startTransition` unhandled) because an audit named those two — and nobody asked how many forms were in the class. CANON-103 says a finding in an enumerable class must enumerate the whole class; it was not applied. **Do not fix this one alone**: sweep every `useActionState` / `<form action>` in `app/**` and `apps/mobile/**`, list per form which fields are uncontrolled, and close them together. |
| **The installed APK's QR encodes `dim-staging.vercel.app`, not the brand domain** | small but public-facing — seen on the device 2026-09-16 | The credential screen of a `preview` build renders the QR over `https://dim-staging.vercel.app/p/{token}`, so an animal wearing that credential sends whoever scans it to the Vercel hostname rather than to `mimar.com.ar`. Consistent with the cutover debt that says build profiles bake their origins (`docs/ops/cutover-debts.md`, Mobile), and recorded here because it is visible to anyone who scans a demo animal today, not only at cutover. |
| ~~**maplibre-gl 5.24 tiene dos XSS criticas y la migracion a 6 dibuja un mapa vacio**~~ | **CERRADO, verificado 2026-09-18** — commit `6571de994`, 2026-09-17; `package.json` fija `maplibre-gl` en `^6.10.0` | El mapa vacio se arreglo: el worker pedia una URL que webpack nunca emitia. La fila queda entera abajo como registro del diagnostico previo: serious, aceptada a sabiendas — PO 2026-09-16; la migracion esta hecha y empujada en la rama `deps/maplibre-v6` (commit `fb294a45b`), NO mergear | No hay parche en la linea 5: el primero arreglado es 6.4.1. La migracion completa compila en frio sin errores, sin un solo `any`, pasa la compuerta entera (1527 archivos, 0 fallas, 69 cercas, peso de rutas en tolerancia) y **dibuja un mapa vacio**. **Medido con un control**, no deducido: mismo arbol, misma base, mismo operador sembrado, mismo alcance (CABA - Palermo, comunas, escala 3 km); lo unico distinto fue la version compilada. La v5 dibuja los poligonos de los barrios y la costa, la v6 solo las etiquetas (marcadores HTML, no dependen de las capas). El mapa reporta `loaded=false`, `isStyleLoaded=false`, 10 capas registradas, 5 fuentes registradas y `querySourceFeatures` en 0 para todas, diez segundos despues de cargar. **Descartado con evidencia**: no es la base (ambas reciben los mismos cuatro GeoJSON estaticos, 6/24/48/513 entidades, mismos bytes); no es WebGL2 (presente en el Chromium de prueba y medido en un Samsung J7 de 2016); no es el manejador de carga (corre: los fetch salen de adentro); no son expresiones legacy (no dibuja ni una capa de color fijo, y la auditoria no encontro ninguna); no es la politica de seguridad (cero pedidos fallidos, cero mensajes de CSP, el worker se crea). **Pista, no causa**: maplibre analiza GeoJSON en un worker; arranca y las fuentes no se pueblan, sin errores. La v6 es ESM-only y dejo de publicar `maplibre-gl-csp.js`, el bundle para politicas estrictas, y la nuestra lo es. **Las dos puertas mas baratas para retomar, en este orden**: (1) buscar el issue — maplibre 6 + Next.js + CSP estricta es una combinacion comun y si es un problema de empaquetado conocido el workaround ya existe; (2) compilar contra `maplibre-gl-dev.mjs`, que trae mapas de fuente, e instrumentar dentro del worker. Lo que falta es VER, no razonar: no es una tarea que se destrabe pensandola mas. |
| **`/perdidas` sometimes fails to load its organization list** | small, intermittent — seen 2026-08-28 → 2026-09-13 | `Failed query` on `organizations` with cause `write ETIMEDOUT` on `/perdidas.rsc` (public page). Unrelated to the analytics-password incident. Low frequency; worth a bounded read + graceful empty state check. |

> **Cutover debts live in their own page: [`docs/ops/cutover-debts.md`](../ops/cutover-debts.md)** — everything tolerated in the single "ensayo" environment that must be fixed before a real launch (credential rotation, separate production, seed data, demo accounts, legal baseline). Opened 2026-09-14; append there, not here.

## PO-gated — not agent work, do not attempt

Listed so you recognise them and hand them over instead of trying.

1. **STALE — superseded 2026-09-12.** Verified in Play Console on 2026-09-13: release **12 (1.0.0)** is "Available to selected testers" on the Alpha closed track since 2026-09-12 11:52, 12+ testers opted in, and testers are signing in. What remains is the 14-day clock (apply for production ~2026-09-26) and one stray **draft** of 12 on the same track, which greys out "Create new release" until it is edited or discarded. Original text: ~~Upload **build 7** to Play (build 6, tree `371a2122`, was never uploaded). Build 5 is published and **cannot sign in** — it shipped without `EXPO_PUBLIC_SUPABASE_*`, which are baked at build time.~~
2. **DONE — confirmed by the PO 2026-09-13**: Data safety → Data types → Photos and videos → **Photos is checked** (the Console wizard hung in automation, so this rests on the PO's own look). Original text: ~~Revise the **Data Safety** form before any build with uploads reaches Play. It declared on 27/08 that the app does not collect photos; that stops being true the moment uploads ship, and a form that no longer matches the binary is a policy violation by itself.~~
...
### Open follow-up: `DEPLOY_REF` still points at the integration branch — CLOSED

**Obsolete, verified 2026-09-18.** `scripts/check-scheduled-fence-refs.ts` has
declared `DEPLOY_REF = "main"` since commit `196c8a278` (2026-09-01), and no
workflow in the repo names `integration/all-20260703` any more. The sequence
this section used to describe (deploy branch → `main`, then `DEPLOY_REF` →
`main`, then delete the pins) already happened.
