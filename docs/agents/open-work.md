...
| 11 | **`<Screen keyboardAvoiding>` is a no-op on Android, for every form screen in the app** | S | `apps/mobile/src/ui/kit.tsx:160` passes `behavior={Platform.OS === "ios" ? "padding" : undefined}`, and a `KeyboardAvoidingView` with no `behavior` renders as a plain `View`. Survivable while Android's own `adjustResize` compensated; under Expo SDK 57 / RN 0.86 the app is edge-to-edge, where the window is no longer resized for the IME. Sixteen call sites pass the prop and none of them gets anything from it — `app/alta.tsx:155`, `app/ingreso.tsx:133`, `src/auth/CrearCuentaScreen.tsx:224`, `src/auth/RecuperarScreen.tsx:138`, `src/auth/IdentidadPendienteScreen.tsx`, `src/adoption/AdoptionApplyScreen.tsx:99`, `src/custody/DevolucionScreen.tsx:169`, `src/custody/MudanzaScreen.tsx:160`, `src/caretakers/CaretakerPetScreen.tsx:179`, `src/transfers/TransferDetailScreen.tsx:198`, `src/transfers/TransferInitiateScreen.tsx:109`, `src/shares/SharesScreen.tsx:250`, `src/lost/LostScreen.tsx:235`, `src/denuncias/DenunciaScreen.tsx:234`, `src/pets/PetProfileEditScreen.tsx:185`, `src/pets/RecordEventScreen.tsx:266`. The fix is one line in `kit.tsx` (`behavior="height"` on Android) plus an emulator pass over the long forms — one change, wide blast radius, which is why it is a row and not a drive-by. Found while landing the native identity form (2026-09-05), which worked around it locally: its two fields sit above the explanatory copy and the return key submits from the last one, so nothing anybody must reach is ever under the keyboard. |
| 12 | **`useScrollToError` never scrolls anything — its provider is out of reach at every call site** | XS | `apps/mobile/src/ui/use-scroll-to-error.ts:33` reads `ScreenScrollContext`, whose provider is mounted INSIDE `Screen` (`apps/mobile/src/ui/kit.tsx:151`). A screen component that renders its own `<Screen>` therefore sits OUTSIDE the provider and reads `null`, so the hook hands back an anchor it can never move. Both callers are in exactly that position: `src/denuncias/DenunciaScreen.tsx:126` and `src/pets/RecordEventScreen.tsx:203` — the two longest forms in the app, which is precisely where it was introduced to help (QOL 2026-09-01). Nothing goes red, because the hook is best-effort by design ("consumers must treat null as nothing to move") and its own unit test wraps the harness in the provider by hand (`src/ui/use-scroll-to-error.test.tsx:44`). Fix: hoist the ref out of `Screen` (a prop, or a provider above it), or let `Screen` accept the anchor. Found 2026-09-05 while landing the native identity form, which left the hook out rather than add a third silent caller. |
