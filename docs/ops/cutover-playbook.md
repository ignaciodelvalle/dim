...
| D3 | **Production domain** → sets `NEXT_PUBLIC_SITE_URL` | **Decided 2026-09-07: `www.mimar.com.ar`.** This row used to say "use a domain you control (`mimar.ar`)" — `mimar.ar` was never registered (NXDOMAIN, no NS, no MX), so following that instruction would have pointed the canonical origin at nothing. The domain the project actually owns is `mimar.com.ar`, and it is where the site already runs. Set `NEXT_PUBLIC_SITE_URL` explicitly anyway; never rely on the fallback. Revisit only if `mimar.gob.ar` is ever delegated. | DNS delegation for a `.gob.ar` needs the partner org; plan lead time. |
...
6. **One canonical origin.** `NEXT_PUBLIC_SITE_URL` is the single source of truth; the hardcoded fallbacks (`www.mimar.gob.ar`, sitemap default) are bugs to converge, not alternatives. The `mimar.ar` fallback that used to head that list was worse than divergent — the domain does not exist — and was moved to `mimar.com.ar` on 2026-09-07; `__tests__/public-hostname-fence.test.ts` now fails on a hostname the project does not own, and on any absolute URL into one of the app's own routes.
