...
| 5 | **Foster volunteers pool** | (plan a escribir) | Pool de owners voluntarios proactivos para foster, con modelo de slots single-use (D16), co-foster opt-in (D17), cascade auto-cancel (D18). Feature nueva grande pero spec auto-contenido. 4 fases A-B-C-D. ~4 días. |
...
| 7 | **Bite-rabies-observation Fases 4-6** (follow-up de F0-F3) | (mismo plan, F4/F5/F6) | F4 org-side reporting (`/org/[orgToken]/mordedura/nuevo`). F5 death-during-observation hook (auto-close con outcome=dead + escalada urgent). F6 `/admin/observaciones` + `/gob/observaciones` para cierre profesional con outcome=positive_rabies. Plus vercel.json cron schedule. ~1-1.5 días. |
...
| `2026-05-18-bite-rabies-observation-design.md` (v1.1) | ✅ Implementado (Fases 0-3 — ver follow-ups) | `plans/2026-05-18-bite-rabies-observation.md` | Bite reporting + observación 10 días. Migración 0021 + columna `pets.rabies_observation_status` (CHECK 5 estados + partial index). 2 event_types nuevos (`rabies_observation_started/ended` libreta-sanitaria; el bite vive en `incident_reported(bite_inflicted)`). Owner self-report en `/mis-mascotas/[token]/eventos/nuevo/mordedura` con banner en el perfil. Surveillance escalation: rabia high-spec durante observación → severity `urgent` + nudge urgente al owner. Cron `app/api/cron/close-rabies-observations` + `closeEligibleRabiesObservations` auto-cierra al día 11 (negative happy path) o flagea para review profesional cuando hubo signals escalables. Capability `bite.report` agregada. **Follow-ups:** (a) Fase 4 org-side reporting `/org/[orgToken]/mordedura/nuevo`; (b) Fase 5 death-during-observation hook (auto-close con outcome=`dead` + escalada urgent); (c) Fase 6 `/admin/observaciones` + `/gob/observaciones` para cierre profesional con outcome=`positive_rabies`; (d) configurar el cron real (vercel.json). Anclaje legal: Decreto 4669/1973 PBA, Ord. CABA 41.831/1987, Res. MS 1144/2018. |
...
| `2026-05-18-bite-rabies-observation.md` | ✅ Implementado (Fases 0-3) | `specs/2026-05-18-bite-rabies-observation-design.md` (v1.1) | F0=schema (migration 0021 + Drizzle + 2 event_types + Zod + libreta classification + `lib/rabies-observation.ts` + capability). F1=`app/actions/bite.ts` con `reportBiteAction` + `ownerCloseRabiesObservationAction` + route `/mis-mascotas/[token]/eventos/nuevo/mordedura` + banner en perfil. F2=hook en `createSymptomObservedAction` para escalation urgent. F3=`lib/rabies-observation-closer.ts` + cron route + CLI script. Fases 4 (org-side reporting), 5 (death hook) y 6 (admin/govt surfaces para cierre profesional) quedan como follow-ups. |
