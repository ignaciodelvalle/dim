...
| `2026-05-17-additional-species-design.md` | 🟡 Parcial — Bloque 1 ✅ Implementado, Bloque 2 (service dogs) pending plan | — | Bloque 1: rabbit / guinea_pig / ferret agregados al `PetForm` via nested select (Otra → Conejo / Cobayo / Hurón / Otro). Solo UI + `speciesLabel` extendida en `lib/format.ts`; sin schema migration, sin nuevos catálogos (vacunas / enfermedades / medicación caen al default branch y retornan `[]` — aceptable para v1). Resolver mapping testeado en `__tests__/species-resolver.test.ts`. **Bloque 2 (service dogs Ley 26.858)** pendiente: tabla sibling `pet_service_dog` + banner de acceso en `/p/[publicToken]` + 3 notification triggers; el spec lista ANDIS / RUPGA / IGDF / ADI como fuentes canónicas. Ley 25.326 → `public_visibility` default a `private_only` por datos sensibles de discapacidad. ~2-3 días cuando se planee. **Bloque 3** (compliance checklist) son notas técnicas — no implementación. |
