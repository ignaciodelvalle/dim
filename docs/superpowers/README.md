...
| 2 | **Welfare moderation admin UI** | (plan a escribir, mini) | Server action `passWelfareToTriageAction` + `confirmWelfareAsSpamAction` ya existen en `app/actions/welfare-moderation.ts`, migration 0029 ya agregó las columnas. Falta page `/admin/maltrato/moderacion` para que el admin vea la queue auto-flag y resuelva. ~½ día. |
...
| 4 | **Validación canonical en `events.ts`** (último de los 5 INDEC) | (mini follow-up) | 4 de 5 actions ya validan (`upgrade.ts:159,326`, `service-offerings.ts:130`, `welfare.ts:97`). Falta `app/actions/events.ts`. ~½ día. |
...
| `2026-05-18-adoption-listing-public-design.md` (v1.3) | ✅ Implementado | — | 6 fases shipped + F5.5 cascade + org-side review surface. Páginas: `/adoptar`, `/adoptar/[petToken]`, `/adoptar/[petToken]/postular`, `/refugios/[orgToken]`, `/mis-mascotas/postulaciones`, `/org/[orgToken]/adopciones`, `/org/[orgToken]/adopciones/[appEventId]`. Helper `lib/adoption-listing.ts` con `queryAdoptionListing` (4 cross-spec guards D18-D21). Apply intent JWT en `lib/apply-intent.ts` (HMAC, 15min TTL, cookie httpOnly). Migrations 0023 (eligibility) + 0030 (listing columns). 3 server actions de application lifecycle (`submitAdoptionApplicationAction` + `approveAdoptionApplicationAction` + `rejectAdoptionApplicationAction`). Cascade auto-rejection en `finalizeAdoptionAction` (F5.5). Después del catalog phase-2 cleanup, los event_types collapsaron a `adoption_application_resolved` (outcome discriminator). |
