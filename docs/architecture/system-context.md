...
<!-- fact:route_handlers -->85<!-- /fact --> `route.ts` handlers, and only
...
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1515<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
...
| Mobile Jest | `apps/mobile/jest.config.js`, <!-- fact:mobile_jest_files -->105<!-- /fact --> files | The phone's own logic, no native modules |
