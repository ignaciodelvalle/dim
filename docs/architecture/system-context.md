...
|---|---|---|
| `pnpm verify` | <!-- fact:verify_fences -->67<!-- /fact --> `lint:*` fences plus `verify:mobile` and `build` | Structure, conventions, authorization scoping, brand casing |
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1489<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
...
| Mobile Jest | `apps/mobile/jest.config.js`, <!-- fact:mobile_jest_files -->78<!-- /fact --> files | The phone's own logic, no native modules |
