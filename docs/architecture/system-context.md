...
Across the whole tree: <!-- fact:pages -->266<!-- /fact --> `page.tsx` files,
<!-- fact:route_handlers -->91<!-- /fact --> `route.ts` handlers, and only
...
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1569<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
