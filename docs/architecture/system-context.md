...
|---|---|---|
| `pnpm verify` | <!-- fact:verify_fences -->66<!-- /fact --> `lint:*` fences plus `verify:mobile` and `build` | Structure, conventions, authorization scoping, brand casing |
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1487<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
