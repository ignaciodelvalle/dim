...
(Biome, over `.` and `app/.well-known`) → <!-- fact:verify_fences -->70<!-- /fact -->
...
<!-- fact:lint_scripts -->70<!-- /fact --> keys in `package.json` start with
`lint:`, and <!-- fact:verify_fences -->70<!-- /fact --> of them are actually
...
`lint:*` script: <!-- fact:check_scripts -->75<!-- /fact --> files under
...
(<!-- fact:mobile_jest_files -->113<!-- /fact --> files, §4), and a config
resolution smoke check.

### Fence categories (representative, not exhaustive)

`pnpm verify`'s <!-- fact:verify_fences -->70<!-- /fact --> fences group into
...
and are the reason CI splits into a `check` job (no DB, most of the <!-- fact:verify_fences -->70<!-- /fact --> fences)
...
<!-- fact:canon_rows -->518<!-- /fact --> rows, harvested from the project's
...
- <!-- fact:canon_enforced -->180<!-- /fact --> **ENFORCED** — a fence or test
