...
(Biome, over `.` and `app/.well-known`) → <!-- fact:verify_fences -->71<!-- /fact -->
...
<!-- fact:lint_scripts -->71<!-- /fact --> keys in `package.json` start with
`lint:`, and <!-- fact:verify_fences -->71<!-- /fact --> of them are actually
...
`lint:*` script: <!-- fact:check_scripts -->76<!-- /fact --> files under
...
`pnpm verify`'s <!-- fact:verify_fences -->71<!-- /fact --> fences group into
...
and are the reason CI splits into a `check` job (no DB, most of the <!-- fact:verify_fences -->71<!-- /fact --> fences)
...
- <!-- fact:vitest_files -->1563<!-- /fact --> files Vitest discovers
