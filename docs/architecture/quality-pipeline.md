...
(Biome, over `.` and `app/.well-known`) → <!-- fact:verify_fences -->69<!-- /fact -->
...
<!-- fact:lint_scripts -->69<!-- /fact --> keys in `package.json` start with
`lint:`, and <!-- fact:verify_fences -->69<!-- /fact --> of them are actually
...
`lint:*` script: <!-- fact:check_scripts -->74<!-- /fact --> files under
...
(<!-- fact:mobile_jest_files -->105<!-- /fact --> files, §4), and a config
resolution smoke check.

### Fence categories (representative, not exhaustive)

`pnpm verify`'s <!-- fact:verify_fences -->69<!-- /fact --> fences group into
...
and are the reason CI splits into a `check` job (no DB, most of the <!-- fact:verify_fences -->69<!-- /fact --> fences)
...
<!-- fact:canon_rows -->517<!-- /fact --> rows, harvested from the project's
...
- <!-- fact:canon_enforced -->179<!-- /fact --> **ENFORCED** — a fence or test
