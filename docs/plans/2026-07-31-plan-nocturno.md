...
| A7 los 7 fallos solo-en-CI | **CERRADA** — ninguno era ambiental: 6 por grieta de nivel de fixture, 1 por copy que D.3 renombró. Dos defectos reales: un gate de a11y verde escaneando un 404, y una limpieza rota que auto-skipeaba un seam crítico | `342e2921`, `51b2eff1` |
| A8 colisión de semilla que rompía CI | **CERRADA** — `select … limit` sin `ORDER BY` → orden físico del heap, que los UPDATEs reubican. **Ordenar solo lo habría empeorado a fallo del 100%**; el arreglo es `NOT EXISTS` con el predicado del índice | `010c2090` |
| A9 crisis-seams (b) y (c) | **CERRADA** — (b) pasaba vacuamente (la semilla ya traía el texto); ahora exige que suba el conteo y que la fila lleve provenencia verificada. (c) deriva de ruta por la fusión F1 | `9ee62a26` |
| A11 cobertura de alert-subscriptions | **CERRADA** — el control de dueño EXISTÍA, solo sin verificar. Hueco nuevo: `jurisdictionProvince` sin `z.enum` deja pasar un error crudo de Postgres al usuario | `6da486e8` |
| A12 los 21 fallos de e2e en CI | **CERRADA** — 6 causas, no 3. `final-seams` **no podía fallar** (verde-y-muerta en TODAS las corridas). El limitador tapaba un supuesto defecto D4 | `b6e4f029`, `a38dffd3`, `96c8a2d3`, `889c481d` |
| A13 la tarjeta de firma nunca se limpiaba | **CERRADA** — preguntaba si la fila del DUEÑO estaba firmada, y append-only hace esa condición inalcanzable. El test mockeado era el habilitante: alimentaba una fila que la query no puede devolver | `2b1ef259` |
| B2 pasada 703 | pendiente | |
| B3 SC-7 | pendiente | |
| **A14 nav drop** | **NO CERRADA, mecanismo hallado**: el fiber root queda `pendingLanes == suspendedLanes == warmLanes` con `pingedLanes == 0` → **una transición suspendida sobre una promesa que no resuelve**. Explica los dos síntomas. `PendingSignaturesCard` es el factor causal dominante (0/8 vs 8/8 con la tarjeta apagada), no solo correlación. Queda un 25-40% de fondo sin tarjeta. Próximo paso: separar la query del render | |
| **A15 (NUEVA) fixture de RLS elige mascota con transferencia pendiente** | **PENDIENTE, solo local**: `__tests__/rls/matrix.test.ts:222` choca con `one_pending_per_pet` cuando la mascota que elige ya tiene una pendiente (`DIM-DEMO-0001`, del seed del 26/07). **CI pasa** — base limpia sin data de demo. Mismo patrón que A8: elegir con `NOT EXISTS` sobre el predicado del índice, como `seed-case-guards.ts` | |
| S1 SC-6 | stretch | |
