...
| **L-10** | **Almirante Brown (BA) tiene polígono y cero localidades** en `ar_localities`: el cuarto partido del GBA se pinta permanentemente como "Sin datos" y toda mascota radicada ahí cae en el residual | **ABIERTO — y NO VERIFICABLE desde el árbol** (17/09). Es dato de referencia externo: el catálogo lo carga `scripts/import-indec-localities.ts` desde el CSV vivo de datos.gob.ar, y en el repo sólo hay una fixture de 9 filas sin `06028`. `0148_manual_alias_localities.sql` tampoco agrega alias. Para cerrarla hace falta una de dos: `SELECT … FROM ar_localities WHERE department_code='06028'` contra la base, o bajar el CSV upstream y buscar `departamento_id="06028"` |
