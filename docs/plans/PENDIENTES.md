...
| **L-22** | **Dos tipos de expediente no tienen NINGUNA vía de cierre**: `outbreak_investigation` y `microchip_remediation` declaran `terminalEvents: []` **y** `manualCloseAllowed: false`, así que ni un hecho los termina ni el operador puede darlos por terminados. Los comentarios de ambos archivos decían "closed manually via case action" — cierto hasta que #41 (10/08) ató el cierre manual a `manualCloseAllowed`. Pesa más en brotes: una investigación de brote es legalmente sensible y puede durar semanas, así que alguien VA a querer cerrarla. `microchip_remediation` tiene cero filas, `outbreak_investigation` no se midió | **ABIERTO — necesita decisión del PO**, no código. Encontrado mirando la pantalla en staging, no el código. Ya no falla en silencio: los comentarios dicen la verdad y la pantalla explica que no hay vía de cierre definida y que hay que pedir la política, en vez de insinuar que algo la cierra sola |
