...
| 3 | **Toggle de leaked-password protection** | **DIFERIDO con razón** (2026-08-05): el dashboard de Supabase lo exige **plan Pro** — no es "un click", es una decisión de facturación. Queda para cuando el proyecto suba de plan (o antes del cutover a producción, donde sí corresponde pagarlo). Es A9 de la cola vieja del 24/06. **Nota 2026-09-18**: verificado contra la API de Supabase — la organización YA está en plan Pro, así que el toggle es ahora una acción del PO sin bloqueo de facturación (listado en `docs/handoff/rumbo-al-piloto.md` §7) |
...
| # | Qué | Estado |
|---|---|---|
| **D4** | ~~**HEIC: se transcodifica en el servidor** (decisión del PO 04/08, ratificada **05/08**). **DIFERIDO ENTERO**~~ **DECISIÓN CAMBIADA 2026-09-18**: el PO decidió que las subidas HEIC/HEIF se **RECHAZAN** ("para no guardar datos de ciudadanos que no nos dieron concientemente"). La transcodificación server-side queda descartada. **CERRADO 2026-09-18** (T1-P2, commit `83cc56ece`): HEIC/HEIF se rechazan por tipo, extensión **y bytes** (`lib/media/heic.ts`), con un mensaje que dice cómo mandar un JPG; los dos borrados de metadatos que subían el original al fallar ahora **fallan cerrado** (`lib/infra/welfare-uploads.ts`, `lib/infra/uploads.ts`). **Queda abierto, y es otra fila**: el video (T2-P3) y dos puertas que guardan EXIF en el camino NORMAL, no en el de falla — adjuntos de asientos sin el opt-in y avatares | ~~Transcodificar necesita una librería de imágenes del lado servidor y pega contra el tamaño y el tiempo de ejecución de la función en Vercel. **Consecuencia que queda escrita, no implícita**: hasta que salga, el GPS del domicilio de un denunciante anónimo **sigue viajando** en cada foto tomada con iPhone. La ventana está **abierta y documentada**~~ |
...
|---|---|---|
| **#41** | **Detalle de caso: acciones de operador.** `components/casos/CaseDetailView.tsx` (327 líneas) tiene **cero controles de operador** — verificado el 07/08: cero `<form>`, cero `<button>`, cero `OpButton`, cero `action=`. Ni cerrar, ni escalar, ni asentar una nota. La cola de Casos ahora ordena por urgencia **en SQL** (SC-6, cerrado el 07/08), así que la fila #1 es de verdad el expediente más urgente de la jurisdicción… **y abre a una página muerta**. El arreglo de SC-6 vuelve esto MÁS visible, no menos: antes el operador llegaba al callejón sin salida por casualidad, ahora el producto lo manda derecho | **CERRADO — y esta fila estuvo equivocada cinco semanas.** El alcance lo decidió el PO el 2026-08-10 (nota para los doce tipos; cierre manual sólo donde el ciclo de vida lo declara) y el código salió el MISMO día: `37caca39f`, *"#41 cierra — el expediente mas urgente ya no abre a una pagina muerta"*. Vive en `src/modules/cases/domain/available-actions.ts` (dominio puro: kind+status → acciones, con la prosa de por qué NO está disponible cada una), `src/modules/cases/application/operator-actions.ts` (`addOperatorNote`, `closeCaseManually`, con el orden mutación-antes-que-evento resuelto contra la carrera), `app/actions/case-operator.ts` y `components/casos/CaseOperatorActions.tsx`, renderizado en `CaseDetailView.tsx:297` detrás de `viewerRole === govt || admin`. **POR QUÉ ESTA FILA MINTIÓ, que es la parte que hay que llevarse**: contó `<form>`/`<button>`/`OpButton` en `CaseDetailView.tsx` y los controles viven en su HIJO. Cero botones en el padre es un dato cierto y una conclusión falsa — la misma forma que L-22 y L-1. Costó una pregunta al PO sobre una decisión que ya había tomado. ~~**Lo único que falta de #41 es ESCALAR a mano**: hoy escalar existe sólo por reloj (cron a 365 días para disputas, 7 para decomisos trabados); un operador que ve que hay que subirlo hoy no tiene cómo. No es "poner botones": cada tipo de caso tiene su propio cierre legítimo (`src/modules/cases/domain/lifecycles/<kind>.ts` declara los eventos terminales) y hay 12 tipos. Las preguntas que hay que responder antes de escribir código: (a) ¿qué subconjunto de tipos gana controles en la primera entrega —¿sólo `bite_incident` + `welfare_denuncia`, los dos que más tráfico tienen?—; (b) ¿el operador cierra, o sólo escala y asienta nota?; (c) ¿la acción vive en el detalle, en la cola por lote (ya existe `OpBulkBar`), o en ambos?~~ **CERRADO 2026-09-18.** El PO decidió el 2026-09-17: escalar a mano, en los 12 tipos, derivado y no literal — sólo cinco tipos declaran `escalated` entre sus `statusValues`, y donde no existe el motivo dice que NO EXISTE el estado, no "todavía no". Código en el mismo commit: `app/actions/case-operator.ts`, `components/casos/CaseOperatorActions.tsx`, `src/modules/cases/application/operator-actions.ts`, `src/modules/cases/domain/available-actions.ts`, migración `0224_case_operator_audit_actions.sql`. Commit `19a327c78`, "feat(casos): las tres decisiones del PO, y dos cosas peores que encontraron". Origen: auditoría 26/07 rojo **#2** (y gate vivo del 28/07, punto #7). Nunca salió de "stretch" en ningún plan |
...
|---|---|---|
| **L-1** | **El despachador de crons se starvea a sí mismo.** Los jobs #2 y #3 declaran 45s cada uno contra un presupuesto de 55s y un lambda de 60s, y el presupuesto se chequea ANTES de cada job, nunca durante. Un kill por timeout no es excepción de JS, así que `withCronRun` deja la fila en `running` para siempre sin disparar alerta. Los jobs 3-22 no corren **ni se marcan como salteados** — incluidos la purga de retención de escaneos (#20, TTL 90 días, privacidad) y `cron_health` (#22), que es justamente el que detectaría la staleness | **CERRADO — verificado el 17/09 contra el árbol.** El código ya no es el que describe esta fila: `app/api/cron/daily/route.ts` reparte con `fairShareMs(ctx.budgetLeftMs, ctx.jobsLeft, BUDGET_MS)` — cuota justa del presupuesto que QUEDA entre los jobs que QUEDAN, no 45s declarados por cabeza — y los salteados se marcan con estado `skipped_budget`, que la corrida filtra y reporta. El modo de falla que la fila temía (jobs 3-22 sin correr **ni** marcarse) ya no existe |
| **L-2** | **El 98,4% de las notificaciones es best-effort sin reconciliación.** 138.553 de 140.766 filas son inserts directos con `catch` que sólo loguea, fuera de la transacción del hecho. No existe job que compare hechos contra notificaciones: "el hecho existe y el aviso no" **no lo descubre nadie**. El carril legal (`event_notification_outbox`) sí es transaccional y está bien resuelto | **ABIERTO**, con dos precisiones del 17/09. (a) El 98,4% (138.553/140.766) es medición de BASE y no se puede comprobar desde el árbol; lo que sí se comprueba es `scripts/notifications-service-baseline.json`, que sigue con **63 archivos** insertando directo. (b) "Fuera de la transacción" **no aplica a todos**: varios sitios baselineados insertan con `tx.insert` DENTRO de la tx del hecho (`verify-dni.ts`, `amend-event.ts`, `foster-repository.ts`). El hueco real sigue siendo que no existe job que compare hechos contra avisos — `notification-reconcile.ts` reconcilia a tiempo de LECTURA contra el estado de la mascota, y `empty-fanout-trace.ts` cubre sólo el fan-out de CERO destinatarios |
| ~~**L-3**~~ | **`check-authz-guards` no puede ver los 33 `route.ts`** — descubre archivos por la directiva `"use server"` y los Route Handlers no la tienen. Instancia viva: `app/api/mis-mascotas/[publicToken]/libreta-export/route.ts` autoriza sin chequear `deletedAt`, así que un titular que ejerció supresión ARCO **sigue pudiendo bajar el PDF completo** de la libreta | **CERRADO 2026-08-21**, las dos mitades. (1) El fence ve **los 47 `route.ts`**, no 33, desde `32d33345` (D4): descubrimiento propio (`listRouteHandlerFiles`, con piso de no-vacuidad de 40 archivos para que un glob que deja de matchear no se lea como corrida limpia), y cada handler exportado llama un guard o lleva un `@no-auth-required: <razón>` escrito. Una forma de export que el analizador no puede leer es **offender**, no un pase silencioso — incluidas las que no nombran ningún método (`export const { GET, POST } = handlers`, `export * from "./impl"`), agregadas hoy tras medir que daban cero offenders. Punto ciego que queda declarado, no tapado: un nombre de guard adentro de un **string literal** cuenta como llamada (`strip-comments.mjs` conserva strings a propósito); cero ocurrencias hoy. (2) La instancia viva se arregló hoy: `libreta-export` pasa por `requireLiveUser()` — mantenimiento → 503, institucional desactivada → 403, sin sesión o **cuenta borrada** → 401 — con test RED→GREEN en `__tests__/libreta-export-route.test.ts` (antes del fix devolvía 200 y la libreta completa) |
...
| **L-5** | **Atender un turno no notifica al dueño.** El gemelo walk-in sí lo hace desde el 16/07 (`notify-owners-of-clinical-event.ts`, escrito exactamente para eso), y el diseño original lo especificaba | **ABIERTO, y es MÁS GRANDE de lo que decía esta fila.** Verificado el 17/09: son DOS casos de uso, no uno. `mark-appointment-attended.ts` y `mark-appointment-no-show.ts` tienen los dos **cero** referencias a `notifications`, y `app/actions/attendance.ts` sólo revalida rutas en ambos caminos. Lo que lo vuelve indefendible es el vecino: en el MISMO módulo, `cancel-appointment-by-org.ts` **sí** inserta la notificación. Así que cancelar avisa, atender no avisa y ausente no avisa. La reja `check-atender-owner-alerts.ts` cubre sólo la ruta del walk-in, por eso ninguno de los dos huecos la despierta |
...
| **L-7** | **Panorama: el sello de frescura del cubo llega a un solo componente**, dentro de un panel que hay que abrir. El PNG y el informe imprimen "Datos al {hoy}" sobre un cubo de hasta 26h. Es un dashboard leyendo un caché y presentándolo como el log — lo que el invariante #3 prohíbe | **CERRADO** `0543fc84c` ("WP5 L-7 — los exports dejan de mentir frescura"). `panorama-export.ts` y `panorama-informe.ts` imprimen "Datos precalculados al {sello}" cuando hay `cubeBuiltAt`, y el sello viaja desde `build-panorama-board.ts` hasta el pie de KPIs. Verificado el 17/09 |
...
| **L-10** | ~~**Almirante Brown (BA) tiene polígono y cero localidades** en `ar_localities`: el cuarto partido del GBA se pinta permanentemente como "Sin datos" y toda mascota radicada ahí cae en el residual~~ | **FALSO EN ENSAYO — CERRADO 2026-09-18.** SQL de sólo lectura contra la base de ensayo (Supabase `agnwyifsdxxoznodutgq`) encontró la fila `indec_id 06028010`, localidad "Almirante Brown", `department_code 06028`, categoría `componente`, fuente `indec_cppdyl` (los partidos del Gran Buenos Aires son filas únicas "componente" en el listado de localidades de INDEC, 57 filas así en AR-B); 10 mascotas activas resuelven a esa localidad por `locality_id`. El "cero localidades" de esta fila salía de la fixture local de 9 filas, que efectivamente no tiene `06028` — pero la base viva sí. **Residual, no fila propia**: quien tipee el nombre del PARTIDO (Burzaco, Adrogué, Claypole…) en vez de "Almirante Brown" depende del mecanismo de alias de `0148_manual_alias_localities.sql`; no verificado acá, queda como observación |
...
| **L-13** | **`SuccessScreen` falta en 4 trámites de peso**: aceptar transferencia de custodia, decidir postulación de adopción, aceptar devolución, y crear un decomiso (que tiene el `publicCode` —el dato mismo del comprobante— y lo tira a la URL). El mandato prohíbe el redirect silencioso tras el submit final en estos flujos (`AGENTS.md:1343-1347`, más el checklist de `:1315`: *"trámites end in SuccessScreen, never a mute redirect"*). **Ojo con la cita**: esta fila decía `AGENTS.md:1299` y ese número ya no resuelve — anclar en números de línea es la misma deriva que rompió baselines tres veces en este repo | **ABIERTO** |
...
| **L-17** | **El QR de check-in es una fachada**: codifica `mimar://appointment/APT-…`, un esquema que nadie registra (cero `protocol_handlers`, cero ruta, cero handler), y el código de respaldo que la pantalla ofrece dictar no lo acepta ninguna superficie — `/org/…/atender` sólo admite `DIM-XXXX-XXXX` | **ABIERTO, con una mitad cerrada.** Verificado el 17/09: sigue cierto que `mimar://appointment/{token}` no resuelve a ninguna pantalla y que el lector del mostrador no existe — `apps/mobile/app/+not-found.tsx` lo dice textual (*"the front-desk payload whose reader does not exist yet"*). Lo que SÍ se cerró es el aterrizaje: ese link ya no cae en el "Unmatched Route" con pista de desarrollador sino en una pantalla honesta en castellano. **Falta el lector, no el aterrizaje** |
...
| **L-19** | **`MapDataTable` no es alcanzable por teclado** (`max-h-80 overflow-auto` sin `tabIndex` ni `role`) — y es el componente que existe específicamente para ser el equivalente accesible del mapa, cuyas etiquetas de división son `aria-hidden` | **CERRADO — verificado el 17/09.** El contenedor `max-h-80 overflow-auto` de `MapDataTable.tsx` es hoy un `<section tabIndex={0} aria-label={caption}>` con su `biome-ignore` documentado, test propio en `MapDataTable.test.tsx`, y la regla axe `scrollable-region-focusable` listada en `report-panorama-a11y.ts` |
| **L-20** | **Los tres harness de QA de Panorama no están cableados** a `package.json` ni a CI: `qa-panorama-chaos`, `report-panorama-a11y`, `qa-panorama-vis`. Corren sólo a mano — y hasta el 10/08 ninguno podía siquiera loguearse | **CERRADO EN LO QUE PEDÍA, con un matiz que vale escribir.** Los tres están cableados: `qa:panorama-a11y`, `qa:panorama-chaos` y `qa:panorama-vis` en `package.json`, y `.github/workflows/panorama-qa-nightly.yml` los corre a las 06:30 UTC contra el stack local. **El matiz**: el propio workflow se declara *report-only* — no puede poner una corrida en rojo. Están cableados; no son una reja. Si lo que se quería era que frenaran algo, eso sigue sin existir y merece su propia fila, no reabrir ésta |
| **L-22** | **`microchip_remediation` no tiene ninguna vía de cierre**: declara `terminalEvents: []` **y** `manualCloseAllowed: false`, así que ni un hecho lo termina ni el operador puede darlo por terminado. Tiene cero filas. **CORREGIDO el 17/09 el otro medio de esta línea**: decía lo mismo de `outbreak_investigation` y era falso. Los dos flags son correctos y el cierre existe igual — `closeInvestigation`, con dos botones en `app/gob/vigilancia/investigaciones/[caseCode]/InvestigationActions.tsx`, que exige outcome (`resolved`/`dismissed`), un motivo de diez caracteres y, para `resolved`, un informe epidemiológico final, con `case_closed` + fila de auditoría en una transacción. `manualCloseAllowed` está en false A PROPÓSITO: prenderlo agregaría una segunda puerta, más débil, al mismo acto sobre un expediente legalmente sensible. **La lección que deja: dos flags en falso no suman una capacidad ausente** — antes de decir que una política está sin escribir, buscar la pantalla | **CERRADO 2026-09-17 — y de paso encontró un TERCER caso de su propia lección.** `microchip_remediation` pasa a `manualCloseAllowed: true` por decisión del PO: el operador lo cierra desde el detalle genérico con un motivo. Va al genérico y no a una pantalla propia con intención — el argumento que mantiene el flag apagado en brotes es no darle una segunda puerta, más débil, a un acto legalmente sensible, y una remediación de microchip es administrativa, no sanitaria. **La decisión casi fue la contraria**: el PO se inclinaba por retirar el tipo y preguntó si tenía uso real. Lo tiene y está vivo — lo abre un `microchip_replaced` con razón `fraud_detected` o `duplicate_detected`, las dos seleccionables hoy en el formulario de admin y en el de la organización veterinaria, con notificación propia y un cuadro en `/gob/usuarios` que cuenta esos reemplazos. Las cero filas no dicen que sobre: dicen que nadie declaró fraude de microchip todavía. **EL TERCER CASO, encontrado mientras se arreglaba éste y más grave que él**: `welfare_denuncia` estaba en el MISMO estado — `terminalEvents: []` + `manualCloseAllowed: false` — y su cierre existe entero desde hace meses: `closeWelfareReport` ← `closeWelfareReportAction` (admin/govt con alcance de jurisdicción) ← `app/gob/maltrato/[id]`, con notas de resolución obligatorias y escritura doble a `cases.status`. O sea el producto le pedía **al kind con más tráfico de la cola** que reclamara una política escrita y desplegada. Arreglado con `dedicatedCloseProse`, el mismo campo que curó brotes. **Y la reja que faltaba, que es lo que impide un cuarto caso**: `__tests__/case-available-actions.test.ts` ahora fija que NINGUNO de los doce kinds emite la frase "todavía no tiene una vía de cierre definida", y que cada uno declara cómo se cierra (hecho terminal, cierre manual, acción de las partes, o pantalla dedicada). La rama que emite esa frase se queda en el código a propósito: es donde caería un kind NUEVO sin política, y ahora eso pone el gate en rojo en vez de mandar a producción una pantalla que miente. La mitad de brotes se cerró: el detalle genérico ya no le pide a una autoridad sanitaria que reclame una política ya escrita, la manda a la pantalla que cierra (`dedicatedCloseProse`) |
| **L-23** | **El reset de React 19 se lleva lo que la persona cargó, y en varios formularios aterriza en un valor EQUIVOCADO, no en vacío.** Un `<form action>` se resetea cuando la acción termina, **también con error**: el texto controlado sobrevive, pero un `<select>` vuelve a su primera opción **aunque esté totalmente controlado**, y un checkbox/radio vuelve a su valor de montaje. Barrido completo el **17/09**: de 90 archivos con `useActionState`, 11 ya usaban `useKeptFields`, 5 eran tests, y quedaron **74 revisados uno por uno**. La lista completa, ordenada por DAÑO y no por archivo, está en engram (`forms/react19-reset-data-loss-inventory`). Lo que importa del orden: la categoría grave no es "pierde campos" sino **el reset escribe una respuesta distinta** — `CorrectSpeciesForm` vuelve a la especie MAL que la persona está corrigiendo, `TransferCustodyForm` vuelve "Dueño/a permanente" a "Custodia temporal", `EndFosterForm` atribuye el cierre a la organización, `PetForm` deshace una corrección de sexo. Un valor equivocado se parece a una respuesta | **PARCIAL — cuatro cerrados el 17/09, todos verificados por mutación.** `DeathRecordForm` (registro de fallecimiento, ~15 campos, el propio form dice que no se puede editar ni deshacer) arreglado, más los tres del grupo "valor equivocado": `CorrectSpeciesForm`, `TransferCustodyForm` y `EndFosterForm`. **Siguen abiertos**, en orden: `PregnancyEndedForm` (pérdida del 100%), `WelfareReportForm` (tiene un `kept()` a mano que NO cubre selects), `FinalizeAdoptionForm`, `CrearConsultorioForm` (wizard de 3 pasos en un form: un error en el paso 3 borra los 1-2), y la cola larga. `LegalMetadataFieldset` es palanca: lo montan las ~11 pantallas de `/gob/reglas`, un arreglo cubre todas |
| **L-24** | **No hay ninguna reja que busque credenciales en el arbol, y las dos casillas de GitHub que lo cubririan NO EXISTEN para este repo.** Medido el 17/09: la API acepta `secret_scanning_non_provider_patterns` y `secret_scanning_validity_checks` sin error y devuelve `disabled`; la pantalla Settings > Advanced Security > Secret Protection muestra solo 'Secret Protection' y 'Push protection', las dos ya activas, sin interruptores para las otras dos. Son de GitHub Secret Protection, que no aplica a un repo publico de cuenta personal. Lo que SI esta prendido cubre patrones de PROVEEDOR (tokens de AWS, claves de Stripe); lo que queda descubierto es una contrasena suelta o una cadena de conexion — exactamente la forma de la deuda ya anotada en docs/ops/cutover-debts.md. Ninguna de las 71 rejas del repo busca eso | **ABIERTO, a proposito.** NO es deuda de cutover: esa lista es para lo tolerado en el ambiente de ensayo hasta produccion, y una reja que falta es trabajo pendiente comun. Tampoco es urgente hoy: el riesgo es una credencial NUEVA, y hoy escribe una sola persona con proteccion de push activa. **El disparador para hacerla es que entre alguien mas al repo** — el piloto municipal o una colaboracion. Una reja local no desublica lo ya publicado; previene lo proximo |
| **L-21** | **El build depende de que Google Fonts conteste, y si no contesta el deploy muere.** `app/layout.tsx` trae las tres familias por `next/font/google`, que las descarga de `fonts.gstatic.com` **en tiempo de build**. Observado en vivo el 2026-08-10 22:04 UTC: el job "Lint, typecheck, build" de CI falló entero con `Failed to fetch 'Encode Sans' from Google Fonts` — nada que ver con el commit. Next las auto-hospeda DESPUÉS de bajarlas, así que en runtime no hay dependencia; la hay en el único momento en que duele, que es publicar. El mismo corte durante un `deploy:staging` deja a los testers sin ambiente | **ABIERTO.** El arreglo estándar es `next/font/local` con los `.woff2` versionados. No se tocó hoy: cambia el pipeline de tipografía y merece su propia verificación visual, no un parche a las 22:30 |
...
| ~~**E-2**~~ | **El radio de chip no tiene 5 valores: tiene 6 geometrías.** El "5" del documento midió sólo CSS. Barriendo la familia entera (30 archivos con nombre de chip/badge/pill/tag/flag/crumb más las reglas de `globals.css` con forma de etiqueta): píldora 9999px (10 miembros, todos de superficie ciudadana), `999px` literal (1 — desvío de **grafía**, rinde idéntico), 6px (1), 5px crudo (1), 4px (4), 3px crudo (4), 2px (3) | **CERRADO 2026-08-10, y la decisión del PO se ejecutó al revés de como estaba escrita — con motivo.** El barrido midió **archivos cuyo NOMBRE contiene chip/badge/pill**, no chips: tres de las seis "geometrías" no son chips. `rounded-[1px]` es el **punto de estado "perdida"**, donde la forma es la codificación redundante no-cromática para daltonismo (volverlo píldora borra una affordance de accesibilidad); `rounded-lg` es el **panel desplegable** de `GovtJurisdictionsChip`, no el chip; `rounded-2xl` es una **tarjeta** con encabezado y cuerpo (`PppPublicBadge`). Es la misma clase de defecto que esta corrida encontró toda la noche: una medición que declara una propiedad más angosta que la que su nombre promete. Y "converger todo a píldora" **contradice la decisión X2-S2 del PO del 2026-07-29** (`app/globals.css:138-153`): dos niveles y sólo dos, ciudadano → píldora, operador → rectángulo institucional para tablas densas. Convergido lo que SÍ era deriva: los cuatro `rounded-[3px]` tipeados a mano pasan a `--radius-op-chip` (mismo píxel, un solo lugar de donde mover), y `rounded-2xl` pasa a `--radius-card`. `__tests__/chip-radius-doctrine.test.ts` fija la doctrina y el recuento corregido para que la próxima auditoría no vuelva a contar seis. **RATIFICADO POR EL PO 2026-08-10 (noche):** se mantiene la doctrina de dos niveles; el rectángulo del nivel operador se queda. No volver a abrir esto sin una decisión nueva |
