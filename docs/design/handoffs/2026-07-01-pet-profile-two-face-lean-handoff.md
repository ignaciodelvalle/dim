...
> **Addendum (2026-07-02, pet-document-redesign S2/S3):** the `LostCockpit` / `DeceasedView` early-return statement above is SUPERSEDED. `LostCockpit` was deleted — a `lost` pet now renders the same profile with `<LostCaseBlock>` as the first, urgent item in `PetAlertStrip` (no separate full-screen cockpit page). `DeceasedView` was likewise replaced by the in-memoriam mainline: a deceased pet renders the same document with the `memorial` skin (sepia filter + ribbon on `CredentialFace`), not an early return either. See the design-system audit (engram `design-system/audit-2026-07-02`, finding #12) and AGENTS.md rule 5 for the current, authoritative block order.
