...
6. **Post-provision verification** — counts public functions/indexes/triggers, `jurisdictions_census` rows, required extensions, and `authenticated` grants against a manifest of minimums. If ANY is short, the provision **aborts here** (before baseline/seeds) with a clear report, so an incomplete schema is never trusted.
