...
- **Email confirmations** (Auth → Settings): ⚠️ **DO NOT enable "Confirm email" yet.** It would close the residual signup session-cookie oracle, BUT the current two-step onboarding assumes a live session after step 1 — with confirmations ON, step 2 dead-ends and the user loses their typed name/DNI (audit finding, task #65). Leave OFF until the onboarding flow is reworked. The code-side enumeration (string-branch) is already fixed; app-layer rate limiting on signup is already shipped.
  - **Re-price this against custody, not only against enumeration (audit A09-1, 2026-09-02).** `pet_transfers` and `pet_caretaker_grants` can be addressed to an e-mail with no account behind it, and the accept compares that string. With confirmations OFF, GoTrue issues a session to anyone who registers an address they merely *know* — so the address is not evidence, and the accept moved titularidad. The application now additionally requires GoTrue's `email_confirmed_at` on the e-mail arm of both accepts and hides the pending row from the list reads (`src/modules/transfers/domain/owner-transfer-rules.ts`, `src/modules/caretakers/application/accept-caretaker-grant.ts`). **That guard is a second lock, not this toggle's replacement:** with confirmations OFF, GoTrue auto-confirms at signup and stamps `email_confirmed_at` itself, so the guard is a no-op until this setting is turned ON. Turning it ON is still the fix at the root; the code change only means the door stays shut if the setting is ever flipped back.
...
| `CUBE_READS` | `1` (optional latency optimization) — serves the 5 admin choropleth layers from the precomputed `panorama_cube`. NOTE (updated 2026-07-15): the LIVE rollup that this flag used to be REQUIRED to bypass was made sargable in migration `0146` + the aggregate-then-resolve rewrite of `rollupPetsPerLocality` — a Buenos Aires cobertura whole-province drill now runs sub-second on the live path, well under the 8s budget, so the flag OFF no longer produces a degraded-empty map. Keep it as defense-in-depth / latency trimming (it also short-circuits the DB entirely for eligible admin national+province views). When enabled, schedule the refresh cron — `app/api/cron/refresh-cube` — alongside; cube staleness > 6h falls back to live. Scope: admin only, national+province and whole-province drills; NOT locality drills, NOT the national department view, NOT govt actors, NOT `verifiedOnly` — those always take the (now-fast) live path. Enable locally/staging by setting `CUBE_READS=1` in the environment (plain env var; read via `process.env.CUBE_READS === "1"` in `load-layer-features-cube.ts`). |
