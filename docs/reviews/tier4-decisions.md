...
## Denuncias / welfare — core SOUND; routing key + EXIF strip are gaps

Verified clean: accused owner can't see the denuncia (`HIDDEN_FROM_SUBJECT_CASE_KINDS`),
anonymity honored (reporterUserId nulled, contact fields never autosaved),
cross-jurisdiction read subsumption-correct + no existence oracle, cross-org IDOR
fenced (`ORG_WELFARE_SELECT` omits description/contact), public receipt minimized
(coarse point, masked contact) + rate-limited, IP not spoofable, MPF export gated.
Auto-fixed in-loop: WF-L6 (stale privacy comment).

| # | Sev | Finding | Where | Decision |
|---|---|---|---|---|
| **WF-H1** | HIGH | **Your own decision ("infer locality from the point") is only half-implemented server-side.** `requireCoords:true` enforces a point, but `jurisdictionProvince/Locality` come from CLIENT text (`provinceName/localityName`), merely canonicalized — NOT reverse-geocoded from the coord. Exploit: coords in jurisdiction A + text=B → stored tagged B → B's govt sees an out-of-territory denuncia, A's govt (competent authority) never sees it. Mis-route + suppression vector. Code comment claims reverse-geocode; it's client-only. | `welfare/actions.ts:917-954`, `location-normalize.ts:87` | Add a SERVER-side reverse-geocode of the enforced point and route off THAT (fallback province-wide on failure — your "el fallback basta" decision). |
| **WF-H2** | HIGH | EXIF/GPS strip covers only jpeg/png/webp, but ALLOWED_MIME accepts heic/heif/gif + all video → an iPhone HEIC or a GPS-tagged video keeps its coordinates, served via signed URL on the public receipt → leaks the reporter's location, defeating the anonymity the strip exists for. | `welfare-uploads.ts:11-37,95` | Server-strip via exiftool/ffmpeg, OR reject the un-strippable formats (safe, degrades: no HEIC/video uploads until strip is built). |
| WF-M3 | MED | Anti-abuse throttle is per-IP/per-user only — NO per-target cap. A rotating-IP botnet can flood denuncias against one pet/address; the exact-string 24h dedup is trivially evaded. Each spawns a govt-triage case + can defame the subject. | `welfare/actions.ts:877`, `welfare-moderation.ts:89` | Add a per-`subjectPetId`/geohash throttle or fuzzy dedup? |
| WF-M4 | MED | Org-side create (`createOrgWelfareReportAction`) does NOT pass `requireCoords` (the public path does) → a professional welfare report can persist with no coordinate, contradicting the "must carry a point" invariant. | `welfare/actions.ts:1243` | Enforce `requireCoords` on the org path too? Parity call. |
| WF-L5 | LOW | Auto-flag moderation is fail-OPEN: if `setFlagged` throws (caught+logged), a bot/troll report is never flagged → lands directly in the govt actionable queue. The "never reaches /gob/maltrato" claim overstates the guarantee. | `create-welfare-report.ts:304` | Fail-closed (hold unflaggable reports)? |
