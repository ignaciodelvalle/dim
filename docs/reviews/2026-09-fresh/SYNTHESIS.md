...
- `A09-1` — the transfer-accept e-mail arm is a bare case-insensitive string compare with no `email_confirmed_at` check. (The lens's `enable_confirmations = false` premise is the LOCAL `supabase/config.toml` value, not a verified production one — live staging, checked 2026-09-02, has confirmation ACTIVE: 42/43 users confirmed, 0 unconfirmed-with-login.) **Anyone who knows the addressee's e-mail** can sign up with it and take titularidad of the animal, once their account clears whatever confirmation policy is actually live.
