...
| R6 | Shared contracts | **EXPENSIVE** (closest to CHEAP) | no package boundary; every pure module drags a 4655-line Drizzle schema through `@/*` |
...
- RN-1 B5 = RN-5 B33 = RN-6 #5 = RN-8 #6: ONE credential JSON endpoint
  ...
  staleAfter + per-section degraded contract. Lifts the 1423-line `/p/` loader.
