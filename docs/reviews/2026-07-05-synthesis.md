...
## ⚠️ CORRECTION (post-investigation) — `src/modules/` is REAL, not hallucinated
My first pass dismissed ~37 findings citing `src/modules/**` as hallucinated paths. **Wrong.** `src/` exists — **520 files**, a hexagonal `modules/{domain,application,infrastructure}` layer my initial scan (db/lib/app/components/scripts) missed. Those findings are **real and back in scope**; the `app/actions/*` thin wrappers delegate to `@/src/modules/**` writers. Re-verdict: the src/modules findings need the same spot-check as the rest — do NOT discount them.

## Needs-investigation — VERDICTS (this pass)
- **07-server-actions — CONFIRMED, security-relevant.** `app/actions/business-rules.ts` is `"use server"` and exports `createBusinessRuleWriter/update/delete` that just call the `@/src/modules` inner writer with **no session guard** (VERIFIED lines 53-56). Same shape flagged for pregnancy/chip-match/booking/service-offerings/attendance/slot-materialization. The `lint:authz` guard PASSES because it only matches the `*ForUser/*ForAuthority/*ForOrg` naming pattern — a `"use server"`-exported `*Writer` that accepts a caller-supplied `actorUserId`/`auth` **slips it**. Per-writer check needed: does each derive actor from the session, or trust client params? If the latter → **impersonation/unauthorized-mutation (HIGH)**. Also `slot-materialization.materializeAllActiveSlots` is a `@no-auth-required` server action anyone can trigger. **Fix:** unexport the writers (import from module in tests only) + widen `lint:authz` beyond the `*For*` pattern to any `"use server"` export accepting an actor/subject id.
- **02-projections — CONFIRMED, event-sourcing integrity.** `updatePetProfile` writes `pets.estimatedWeightKg` with **no `weight_recorded` event** (VERIFIED-class: a cache column written outside the event log → not re-derivable → the Tuni drift class, violates Invariant #3). `replayPetMicrochip` ignores `microchip_replaced` → chip projection wrong after replacement; `owner-nudges hasChip` stays true after revocation; the PPP "Faltan datos" card missing on the list view (owner-dashboard omits species/breed/weight). **Fix:** route weight/chip mutations through event writers (or stop writing the cache on profile edit); fold `microchip_replaced` into replay; pass PPP inputs to the list.
- **19-i18n — CONFIRMED, real cluster (~12 surfaces).** Raw enums/UUIDs/snake_case reaching the UI: `OpOmnibox` (VERIFIED line 67 `${r.caseKind} · ${r.status}`), disputas timeline, decomisos preview, AppointmentCard (`cancelled_by_org`→"Confirmado" wrong), outbox source type, SituationalMap tooltip, "User ID (UUID)" labels. Same class as the case-i18n fix (#37) but in surfaces it didn't cover. **Fix:** map each through the existing helpers (caseKindLabel/caseStatusDisplay/eventTypeLabel/ROLE_LABELS/statusLabel).
