...
|---|---|---|
| `{{SHA}}` | El commit sobre el que se escribió el pack, nueve caracteres | Lo que imprima `git -C C:/dev/dim rev-parse --short HEAD` el día que entregás el pack. Ningún commit va escrito acá: un ejemplo miente al día siguiente, que es la misma razón por la que el marcador existe |
