// Lost-pet listing DB query — split out from `lib/lost-listing.ts` so the
...
export async function queryLostListing(
  ...
  for (const row of baseRows) {
    ...
    if (sinceMs !== null && meta.occurredAt.getTime() < sinceMs) continue;

    const rawDescription =
      typeof meta.payload.location_description === "string"
        ? (meta.payload.location_description as string).trim() || null
        : null;
...
// returns just the integer — the DOM strip in Phase 3 will request these
// in parallel along with the listing query.
...
// Re-export `lt` to silence the unused-import biome rule until I wire the
// SQL-side cursor in a follow-up (today the cursor is applied JS-side).
void lt;
