...
// on /gob or /admin used to redirect the operator to bare /login, losing the
...
// `returnTo` into requireUserOrRedirect, so /login can restore it after a
// successful re-login (LoginPage already supports returnTo — see
// app/(auth)/login/page.tsx's safeReturnTo handling; that half was already
...
describe("requireAdminOrGovtOrRedirect — returnTo preservation (finding #13)", () => {
  it("an unauthenticated /gob/* request redirects to /login carrying the full attempted URL (path + query)", async () => {
    ...
    expect(url).toBe(
      `/login?returnTo=${encodeURIComponent("/gob/denuncias?etapa=triage&queue=mine")}`,
    );
  });

  it("falls back to bare /login when the x-full-path header is absent (never throws)", async () => {
  ...
  it("falls back to bare /login when headers() itself throws (e.g. no request context)", async () => {
    redirectMock.mockClear();
...
describe("requireAdminOrRedirect — returnTo preservation (finding #13)", () => {
  it("an unauthenticated /admin/* request also carries the full attempted URL", async () => {
    ...
    expect(redirectMock).toHaveBeenCalledWith(
      `/login?returnTo=${encodeURIComponent("/admin/programa?province=AR-C")}`,
    );
  });
});
