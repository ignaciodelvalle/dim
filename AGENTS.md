...
| **Cada tabla nueva de `public` se declara contra los dos RPC** | `pnpm lint:subject-rights` → `scripts/check-subject-rights-coverage.ts`. Cuatro listas, verificación en las dos direcciones contra `pg_get_functiondef`. **Prueba mención, no corrección del predicado.** 17 tablas siguen en `KNOWN_GAP` — la 0207 cerró `libreta_share_tokens` y la 0208 cerró las tres suyas. **No transcribas este número: sale de `pnpm lint:subject-rights`, que lo computa desde la lista viva** |
