...
1. **Credencial first (Face 1)** — one credential object (LnHero identity + compliance stamp row + printed QR, plus an owner-only Emergencia card with tap-to-call vet/emergency contacts when set) is the first content block in every non-terminal state. No conditional banner precedes it. Provenance gates the stamps (H1): a stamp reads "al día" only for professional/institutional-verified events. There is no separate mono-ID card and no "Inscripción válida" seal block — both were cut as redundant with the hero + stamp row.
