...
export function PetMarkLostFooterCta({ petPublicToken, petStatus }: Props) {
  ...
  return (
    <div
      className="md:hidden fixed bottom-0 inset-x-0 z-30 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-white/95 backdrop-blur border-t border-neutral-200 dark:bg-neutral-950/95 dark:border-neutral-800"
      data-section="marklost-footer-cta"
    >
      <Link
        href={`/mis-mascotas/${petPublicToken}?sheet=marcar-perdida`}
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-600 dark:bg-amber-500 text-white font-semibold text-sm px-4 py-3 hover:bg-amber-700 dark:hover:bg-amber-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-600 focus-visible:ring-offset-2 transition-colors"
      >
