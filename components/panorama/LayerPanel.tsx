...
export function LayerPanel({ states, onToggle }: Props) {
  return (
    <fieldset className="space-y-1.5">
      ...
      <ul className="space-y-1">
        {PANORAMA_LAYERS.map((layer) => {
          ...
          return (
            <li key={layer.id}>
              <label className="flex cursor-pointer items-center gap-2.5 rounded-[6px] px-1.5 py-1 text-[12px] text-ln-op-ink-2 hover:bg-ln-op-card">
                <input
                  type="checkbox"
