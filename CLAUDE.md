...
>
> A **broken file** is one that reported with an error OUTSIDE any test — it is named under the verdict line with its message. A mock/collection/import error there (`No "x" export is defined on the "@/db" mock`) is a real failure with zero failing tests: the file never ran. Four consecutive runs on 2026-08-22, three of them on CI, were exactly that and were misread as the teardown crash because the old verdict counted only tests. The other message a broken file can carry is `Worker exited unexpectedly` AFTER all its tests reported: a worker that died at TEARDOWN. The script re-folds vitest's exit code on purpose so nobody normalises a crashing run, and that is correct: this is an **open defect**, not a tolerated condition — it must be fixed, not papered over. Until it is, a run whose only broken file carries that exact message may be committed — say so in the commit, with the verdict line quoted. Migrations are forward-only and immutable (`db/migrations/NNNN_*.sql`); recount the next free integer at write time — never hardcode one from a plan. Writing a migration file is agent work; applying it to a remote DB is Ignacio-gated.
